...
const Post = ({data}) => {
  ...
  return (
    <div className='flex flex-col px-4 justify-center'>
        <div>
        <Link href={"/"}><a><Image src={img||"/"} width={600} height={400}></Image></a></Link>        </div>
        <div>
        <Link href={"/"}><a className='text-orange-700 text-sm'>{category||"unknown"}</a></Link>
        <span className='text-black'>-{published||"unknown"}</span>
        </div>
        <div>
        <Link href={"/"}><a className='text-base font-bold text-gray-800 hover:text-gray-600 md:text-xl'>{title||"unknown"}</a></Link>
        </div>
        <div>
