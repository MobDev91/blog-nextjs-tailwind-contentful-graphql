...
const Post1 = ({data}) => {
  const {sys,slug,title,subtitle,category,published,postImage,author} = data;
  ...
  return (
    <div className='flex flex-col px-4 justify-center'>
        ...
        <div>
        <Link href={`${baseUrl}${slug}`}><a className='text-xl md:text-3xl font-bold text-gray-800 hover:text-gray-600'>{title}</a></Link>
        </div>
        <div>
