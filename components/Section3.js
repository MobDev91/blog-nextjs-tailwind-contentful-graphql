...
const Section3 = () => {
  ...
  return (
    <section className="container mx-auto px-10">
      <h1 className="text-3xl font-bold text-gray-800 text-center py-10">
        Most Popular
      </h1>
      <Swiper slidesPerView={2} onSwiper={(swiper) => console.log(swiper)}>
        {posts.map((post) => (
          <SwiperSlide key={post.sys.id}>
            <Post1 data={post} key={post.sys.id}></Post1>
          </SwiperSlide>
        ))}
      </Swiper>
    </section>
